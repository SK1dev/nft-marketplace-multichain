...
export default function Home() {
  ...
  */
  ...
  async function loadHHSaleNFTs() {
    ...
    const transaction = await contract.sKDMarketSale(hhnft, nft.tokenId, {
      value: price
    })
  ...
  */
  ...
  async function buyNewGoe(nft) {
    ...
    const price = ethers.utils.parseUnits(nft.price.toString(), 'ether')
    const transaction = await contract.sKDMarketSale(goenft, nft.tokenId, {
      value: price
    })
    await transaction.wait()
    loadGoeSaleNFTs()
  ...
  */
  ...
  async function buyNewBsct(nft) {
    ...
    const price = ethers.utils.parseUnits(nft.price.toString(), 'ether')
    const transaction = await contract.sKDMarketSale(bsctnft, nft.tokenId, {
      value: price
    })
    await transaction.wait()
    loadBsctSaleNFTs()
  ...
  */
  ...
  async function buyNewMum(nft) {
    ...
    const price = ethers.utils.parseUnits(nft.price.toString(), 'ether')
    const transaction = await contract.sKDMarketSale(mmnft, nft.tokenId, {
      value: price
    })
    await transaction.wait()
    loadMumSaleNFTs()
                ...
                return (
                  <Grid xs={3}>
                    ...
                    <Row key={id}wrap="wrap" justify="space-between" align="center">
                      ...
                      <Text  style={{ fontSize: '30px' }}>{nft.val} <img src='skdr-logo.png' style={{ width: '60px', height: '25px', marginTop: '4px' }} /></Text>
                ...
                <Card style={{ marginRight: '3px', boxShadow: '1px 1px 10px #ffffff' }} variant="bordered" key={i}>
                  ...
                  <Card.Footer css={{ justifyItems: "flex-start" }}>
                    <Row wrap="wrap" justify="space-between" align="center">
                      ...
                      <Text style={{ fontSize: '30px' }}>{nft.price}<img src='skdr-logo.png' style={{ width: '60px', height: '25px', marginTop: '4px' }} /></Text>
